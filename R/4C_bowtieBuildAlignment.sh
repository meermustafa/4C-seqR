...
for fq in ${INPUT[@]}; do
    ...
    echo "bowtie -v 1 -m 1 ${BOWTIEIDX} -q ${fq} ${fq}.btie.fq; cat ${fq}.btie.fq | cut -f 3 | sort | uniq -c | sort -rn > ${fq}.trim.bowtie.results " | qsub -V -cwd -m abe -M mmustafa@nygenome.org -N ${fq}_job -pe smp ${THREADS}
